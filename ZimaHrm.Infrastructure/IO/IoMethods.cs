...
namespace ZimaHrm.Core.Infrastructure.IO
{
	...
	public static class IoMethods {
		...
		public static void CopyFolder( string src, string dest, bool overwriteExistingFiles ) {
			...
			foreach( var fsi in di.GetFileSystemInfos() ) {
				...
				if( fsi.GetType() == typeof( FileInfo ) )
                    System.IO.File.Copy( fsi.FullName, destName, overwriteExistingFiles );
		...
		public static long GetFolderSize( string path ) {
			return System.IO.File.Exists( path ) ? new FileInfo( path ).Length : Directory.GetFileSystemEntries( path ).Sum( filePath => GetFolderSize( filePath ) );
		}
